...
// 필수 항목 체크
if (empty($name) || empty($contact) || empty($reservation_date) || empty($diet_type)) {
...
$diet_type = h($diet_type);
$content = h($content);

try {
    $pdo = db_connect();
    
    // diet_inquiries 테이블에 데이터 삽입
    $sql = "INSERT INTO diet_inquiries (name, contact, reservation_date, diet_type, content, created_at) 
            VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if ($result) {
        echo '<script>alert("다이어트 상담 문의가 성공적으로 접수되었습니다."); location.href="/";</script>';
    } else {
