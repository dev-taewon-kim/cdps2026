...
try {
    // hp_inquiries 테이블 존재 여부 확인
    $checkTable = $pdo->query("SHOW TABLES LIKE 'hp_inquiries'");
    ...
    if (!$tableExists) {
        ...
        // 전체 문의 수 조회
        $count_sql = "SELECT COUNT(*) FROM hp_inquiries";
        ...
        // 현재 페이지 문의 조회
        $sql = "SELECT * FROM hp_inquiries ORDER BY id DESC LIMIT :offset, :per_page";
          ...
          <tbody>
            ...
            <?php if (!empty($inquiries)): ?>
              ...
              <tr>
                ...
                <td><?php echo htmlspecialchars($inquiry['hp_type']); ?></td>
                <td><?php echo htmlspecialchars($inquiry['reservation_date']); ?></td>
                <td><?php echo date('Y-m-d', strtotime($inquiry['created_at'])); ?></td>
                <td><a href="./inquiry_hp_view.php?id=<?php echo $inquiry['id']; ?>" class="view_btn">확인하기</a></td>
              </tr>
              <?php endforeach; ?>
            <?php else: ?>
              <tr>
